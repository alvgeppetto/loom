#!/usr/bin/env python3
"""Part 2: Remaining spot-checks (TB rrs/eis, Cholera, Ebola, RSV)."""
import json
import time
import urllib.parse
...
check(
    "Cholera + CRISPR + diagnostic (broad)",
...
check(
    "Cholera + CRISPR + ctxA/cholera toxin",
    '("Vibrio cholerae"[Title/Abstract]) AND CRISPR[Title/Abstract] '
...
print("\n=== MPOX GENE GAPS (quick check) ===")
check(
    "Mpox + CRISPR + thymidine kinase",
    '(Mpox[Title/Abstract] OR Monkeypox[Title/Abstract]) AND '
    'CRISPR[Title/Abstract] AND ("thymidine kinase"[Title/Abstract] OR J2R[Title/Abstract])',
)
